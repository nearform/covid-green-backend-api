...
async function exposures(server, options, done) {
  ...
  const getCertificate = async () => {
    const { rowCount, rows } = await server.pg.read.query(certificateSelect())

    if (rowCount === 0) {
      throw new Error('Certificate not found')
    }

    const [{ keyid, privateKey, publicKey }] = rows
  ...
  server.route({
    ...
    handler: async request => {
      ...
      const { keyid, privateKey } = await getCertificate()
  ...
  server.route({
    ...
    handler: async request => {
      ...
      const { keyid, privateKey, publicKey } = await getCertificate()

      const { jti } = jwt.verify(token, publicKey, {
        audience: options.security.jwtIssuer,
        issuer: options.security.jwtIssuer
      })
