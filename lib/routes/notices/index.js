...
async function notices(server, options) {
  ...
  const {
    noticesRateLimitSeconds: rateLimitSeconds,
    noticesRateLimitRequestCount: rateLimitRequestCount
  } = options.security
  if (rateLimitSeconds == null) {
    console.log(
      'Notice Rate Limit: Off - users may only send a single notice key request'
    )
  } else {
    console.log(
      `Notice Rate Limit: On - users may send ${
        rateLimitRequestCount == null ? 1 : rateLimitRequestCount
      } notice key requests every ${rateLimitSeconds} seconds`
    )
  }
  ...
  async function checkRateLimit(registrationId) {
    const query =
      rateLimitSeconds == null || rateLimitSeconds < 0
        ? registerOnetimeNotice({ registrationId })
        : registerRateLimitedNotice({
            registrationId,
            rateLimitSeconds,
   ...
   *  NOTICES_RATE_LIMIT_SECS == 0: a user is allowed to request N notice key requests with no rate limiting
   *  NOTICES_RATE_LIMIT_SECS > 0: notice key requests are rate limited in the following ways
   *    NOTICES_RATE_LIMIT_REQUEST_COUNT == undefined or 1: a user is allowed to request a new notice key after NOTICES_RATE_LIMIT_SECS has passed
   *    NOTICES_RATE_LIMIT_REQUEST_COUNT > 1: a user is able to request up to NOTICES_RATE_LIMIT_REQUEST_COUNT requests within NOTICES_RATE_LIMIT_SECS
   *
   * If rate limiting is in play then the value of NOTICES_RATE_LIMIT_REQUEST_COUNT will be taken into account. This
   * value allows for a specific number of additional requests within a rate limit period.
   *
   * Responses:
