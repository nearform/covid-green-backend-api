...
function logRateLimit(rateLimitSeconds) {
  ...
  if (rateLimitSeconds === 0) {
    console.log(
      'Notice Rate Limit: Off - users may send how many notice key request as they want'
    )
    return
  }

  console.log(
    `Notice Rate Limit: On - users may send one notice key requests every ${rateLimitSeconds} seconds`
  )
}

function noop() {}

async function rateLimitOnce(server, registrationId) {
  const query = registerOnetimeNotice({ registrationId })
...
async function rateLimit(server, rateLimitSeconds, registrationId) {
...
function getCheckRateLimitFn(server, rateLimitSeconds) {
  if (rateLimitSeconds == null) {
    return rateLimitOnce.bind(null, server)
  }
  ...
  return rateLimit.bind(null, server, rateLimitSeconds)
}
...
async function notices(server, options) {
  ...
  const { noticesRateLimitSeconds: rateLimitSeconds } = options.security

  logRateLimit(rateLimitSeconds)

  const checkRateLimit = getCheckRateLimitFn(server, rateLimitSeconds)
