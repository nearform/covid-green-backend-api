...
async function getConfig() {
  const env = envSchema({
    ...
    schema: S.object()
      ...
      .prop('NOTICES_RATE_LIMIT_REQUEST_COUNT', S.anyOf([S.number(), S.null()]))
  ...
  const config = {
    ...
    security: {
      ...
      noticesRateLimitRequestCount: env.NOTICES_RATE_LIMIT_REQUEST_COUNT,
  ...
  if (isProduction) {
    ...
    config.security.noticesRateLimitRequestCount = Number(
      await getOptionalParameter(
        'security_self_isolation_notices_rate_limit_request_count',
        1
      )
    )
