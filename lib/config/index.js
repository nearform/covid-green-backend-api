...
async function getConfig() {
  const env = envSchema({
    ...
    schema: S.object()
      ...
      .prop('QR_APP_URL', S.string())
      .prop('QR_QUEUE_URL', S.string())
      .prop('QR_BUCKET_NAME', S.string())
  ...
  const config = {
    ...
    security: {
      ...
      qrCodeAppName: env.QR_CODE_APP_NAME,
      qrCodeVersion: env.QR_CODE_VERSION,
      qrCodeJwtSecret: env.QR_CODE_JWT_SECRET,
    ...
    qr: {
      ...
      appUrl: env.QR_APP_URL,
      queueUrl: env.QR_QUEUE_URL,
      bucketName: env.QR_BUCKET_NAME,
  ...
  if (isProduction) {
    ...
    config.cors.origin = await getParameter('cors_origin')
    config.cors.credentials = /true/i.test(
      await getParameter('cors_credentials')
    )
    ...
    config.qr.appUrl = await getParameter('qr_generate_url')
    config.qr.queueUrl = await getParameter('qr_queue_url')
    config.qr.bucketName = await getParameter('s3_qr_bucket')
