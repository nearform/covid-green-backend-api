...
async function getConfig() {
  const env = envSchema({
    ...
    schema: S.object()
      ...
      .prop('ENABLE_LEGACY_SETTINGS', S.boolean())
  ...
  const config = {
    ...
    routes: {
      ...
      legacySettings: env.ENABLE_LEGACY_SETTINGS,
  ...
  if (isProduction) {
    ...
    config.routes.legacySettings = /true/i.test(
      await getParameter('enable_legacy_settings')
    )
